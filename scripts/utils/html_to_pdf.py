...
class HTMLToPDFConverter:
    ...
    async def convert_file(
        ...
        try:
            ...
            # Wait for MathJax typesetting and web-font loading before
            # capture.  CHTML output draws every glyph from MathJax web
            # fonts; a PDF captured before they finish loading drops or
            # corrupts math symbols in the rendered document.
            try:
                math_ready = await page.evaluate(
                    """() => {
                        if (!window.MathJax) return Promise.resolve('none');
                ...
                logger.info(f"MathJax ready: {math_ready}")
            except Exception as e:
                logger.warning(
                    f"MathJax readiness check failed ({e}); proceeding anyway")
            try:
                await page.evaluate("() => document.fonts.ready")
                logger.info("Document fonts ready")
            except Exception as e:
                logger.warning(
                    f"Font readiness check failed ({e}); proceeding anyway")
            # Settle: let the font swap finish painting before capture.
    ...
    def _build_pdf_options(self, options: Dict) -> Dict:
        ...
        # Tagged PDF: Chromium builds the text layer from the document
        # accessibility tree rather than glyph ToUnicode maps, which
        # preserves MathJax CHTML characters (phi, nabla, italic latin)
        # that otherwise drop out of copy/search text.
        if options.get('tagged', True):
            pdf_options['tagged'] = True

        # Outline/bookmarks from headings.
        if options.get('outline', True):
            pdf_options['outline'] = True
