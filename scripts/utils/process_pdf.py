...
def parse_citation_cff():
    ...
    try:
        ...
        # Always parse abstract manually to preserve paragraph breaks
        # (PyYAML folds newlines in single-quoted strings to spaces)
        abstract_match = re.search(
            r"^abstract:\s*'((?:[^']|'')*)'",
            content, re.MULTILINE
        )
        if abstract_match:
            raw = abstract_match.group(1).replace("''", "'")
            # Preserve paragraph breaks (\n\n); collapse single newlines to spaces
            raw = raw.replace('\n\n', '\x00PARA\x00')
            raw = re.sub(r'\n', ' ', raw)
            raw = raw.replace('\x00PARA\x00', '\n\n')
            raw = re.sub(r' +', ' ', raw)
            data['abstract'] = raw.strip()
...
def main():
    ...
    parser.add_argument(
        '--no-compress',
        action='store_true',
        help='Skip Ghostscript compression entirely. Required for '
             'tagged PDFs: gs rewrites the document and destroys the '
             'accessibility-tree text layer, dropping MathJax glyphs '
             'from copy/search extraction.'
    )
    ...
    if args.no_compress:
        ...
        print("Step 1: Compression skipped (--no-compress; preserving "
              "tagged text layer)")
