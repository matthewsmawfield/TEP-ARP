    ...
    try {
        ...
        for (const section of sortedSections) {
            ...
            if (fs.existsSync(componentPath)) {
                ...
                const escapedTitle = section.title.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;');
                componentsHtml += `
                <section id="${section.id}" class="manuscript-section" data-section="${escapedTitle}">
                    ${componentHtml}
                </section>`;
            } else {
                console.warn(`⚠️  Component not found: ${section.file}`);
                const escapedTitleErr = section.title.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;');
                componentsHtml += `
                <section id="${section.id}" class="manuscript-section" data-section="${escapedTitleErr}">
        ...
        const staticContent = indexContent
            ...
            // Use a function replacement to avoid $& pattern interpretation in componentsHtml
            .replace(
