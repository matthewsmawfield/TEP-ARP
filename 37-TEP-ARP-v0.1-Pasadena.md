...
First published: 24 September 2026 · Last updated: 24 September 2026
...
The quantity $A_{\rm int}(Q) = (1+z_G)/(1+z_Q)$ depends on the two measured redshifts alone. No distance estimate, Hubble constant, or cosmological parameter enters its definition. If the two objects are at a common distance, $A_{\rm int}$ measures the companion's local proper-time offset directly; if they are not, $A_{\rm int}$ still quantifies the component of the redshift differential that cannot be attributed to the shared background. The diagnostic is therefore well-defined under either hypothesis, and the physical-proximity tests of Sections 5–6 decide between the two readings.
...
| NGC 1199 – companion | 2MASXi J0303366-153740 | $+0.21$ | $2.4 \times 10^{-4}$ | $0.9$ ($0.45$) | silhouette pair; annulus test below |
...
Three features of the posterior carry physical weight. First, the well depths are individually interpretable: each $d_i$ is the scalar-field offset of that object's local well relative to the shared pair field, and the knot depths recover the direct $\Delta\phi = -\ln A_{\rm int}$ of each object ($0.301$ and $0.188$ respectively)—the same quantity computed independently from the redshift ratios of Section 4, and insensitive to $w$ throughout the allowed range. The companion depth $d_Q = 0.020$ sits slightly below its own $\Delta\phi = 0.026$ because the Lorentzian tail of the adjacent knot well ($x = 0.91$, only $5$ arcsec away along the axis) contributes the difference—the measured depth at the companion position is shared between the two wells, which is precisely the nested-wells physics the model encodes.
...
| Chance-alignment probability | $P_{\rm joint} = 6.5 \times 10^{-14}$ ($9.4 \times 10^{-15}$ under published SDSS density) at archive-measured separations; compact subsample alone ($\theta < 300''$, eight pairs) $7.7 \times 10^{-14}$; axis-conditioned on the five morphology-defined corridors $8.7 \times 10^{-20}$; X-ray-selection-aware at the measured $\Sigma_X = 3.8\ {\rm deg}^{-2}$ gives $1.3 \times 10^{-22}$; after removing the member falsified by the Ly$\alpha$ path-length audit the surviving-ensemble products are $3.6 \times 10^{-13}$, $6.6 \times 10^{-14}$, $4.3 \times 10^{-13}$ (seven-pair compact), $4.9 \times 10^{-19}$, and $7.2 \times 10^{-22}$ respectively; look-elsewhere-corrected catalogue probability $P_{\rm cat}(N) = \prod_i[1-e^{-Np_i}]$ < 0.05 for parent search volumes $N \lesssim 200$ fields ($N \lesssim 310$ axis-conditioned) | improbable under random projection; selection-conditioned |
...
The table is deliberately asymmetric in its burdens: the tests that can refute the framework (absorption ordering, field continuity, chance probability) all return results the framework requires, while the tests that could only bound it (separation scaling, residual whiteness) return measurements consistent with it. The single entry that could have killed the hypothesis—a catalogue severity statistic of order unity, the outcome the standard dismissal predicts—is more than thirteen orders of magnitude the other way.
...
The Temporal Equivalence Principle has been applied to the discordant quasar–galaxy pairs catalogued by Halton Arp. A twelve-pair catalogue drawn from the published literature and verified against the NASA/IPAC Extragalactic Database has been carried through a twenty-five-step reproducible pipeline—archive imaging, redshift verification, bridge transects, absorption ordering, chance-alignment statistics, field-profile fitting, and MCMC inference—and the two hypotheses at issue have been tested against each other at every point where the data can decide. The findings are as follows.
...
- *scripts/steps/*—Reproducible analysis pipeline with 26 registered step scripts organized into five blocks (data ingestion and target catalogue, TEP formalism, physical-connection evidence, field-gradient inference, and synthesis / figures).
...
This executes all 26 registered step scripts in sequence, populating `results/figures/` and `results/outputs/` with fresh data. Individual blocks or steps can be run selectively:
...
The pipeline is fully implemented and all 26 registered steps run successfully against real published data. The pipeline structure, data sources, and analysis methodology are documented in `scripts/README.md`.
...
This generates the static site in `site/dist/` and the markdown version at the repository root (`37-TEP-ARP-v0.1-Pasadena.md`). A PDF can be generated from the built site:
...
- *Tully 2MASS group catalogue* (Tully 2015; VizieR J/AJ/149/171): The 2,822 nests and 14,142 satellites used for the satellite redshift-asymmetry audit step (the signature reported by Arp & Sulentic 1985 and examined further by Girardi et al. 1993).
